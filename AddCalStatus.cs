...
{
    public partial class AddCalStatus : Form
    {
        ...
        private void DisplayData()
        {
            ...
            adapt = new SqlDataAdapter("Select * from tbl_CalibrationStatus", con);
            adapt.Fill(dt);
            dataGridView1.DataSource = dt;
            dataGridView1.ColumnHeadersDefaultCellStyle.Font = new System.Drawing.Font("Calibri", 13, FontStyle.Bold);
        }
        private void AddCalStatus_Load(object sender, EventArgs e)
        {
            DisplayData();
        }
        ...
        {
            dataGridView1.Columns["CalibrationStatus"].Width = 271;
        }

        private void dataGridView1_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ID = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells[0].Value.ToString());
            txtCalStats.Text = dataGridView1.Rows[e.RowIndex].Cells[1].Value.ToString();
        }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            if (txtCalStats.Text != "")
            {
                ...
                SElectDouble_SQL = "SELECT CalibrationStatus from tbl_CalibrationStatus where CalibrationStatus = '" + txtCalStats.Text + "'";
                ...
                {
                    cmd = new SqlCommand("insert into tbl_CalibrationStatus(CalibrationStatus) Values (@calibrationstat)", con);
                    con.Open();
                    cmd.Parameters.AddWithValue("@calibrationstat", txtCalStats.Text);
                    cmd.ExecuteNonQuery();
                    con.Close();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            if (txtCalStats.Text != "")
            {
                cmd = new SqlCommand("update tbl_CalibrationStatus set CalibrationStatus=@calibrationstat where ID=@id", con);
                con.Open();
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (ID != 0)
            {
                ...
                {
                    cmd = new SqlCommand("delete tbl_CalibrationStatus where ID=@id", con);
                    con.Open();
                    cmd.Parameters.AddWithValue("@id", ID);
                    cmd.ExecuteNonQuery();
                    con.Close();
